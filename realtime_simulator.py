...
# ── Config ───────────────────────────────────────────────────────────────
RECORD_ID  = '208'       # try: 100, 106, 208, 212 — each has different rhythms
FS         = 360
WINDOW     = 280
BEFORE     = 100
AFTER      = WINDOW - BEFORE
DISPLAY_SEC = 6          # seconds of ECG shown in the rolling window
...
ax_ecg   = fig.add_subplot(gs[0, :])   # full width — rolling ECG
ax_conf  = fig.add_subplot(gs[1, :])   # full width — confidence bar
ax_hist  = fig.add_subplot(gs[2, 0])   # beat class history
ax_count = fig.add_subplot(gs[2, 1])   # running totals
...
# Current beat label overlay on ECG
...
for peak, sym in zip(r_peaks, symbols):
    ...
    # Simulate real cardiac timing (comment this out to run at full speed)
    rr_interval = (AFTER / FS)    # approx time to next beat
    time.sleep(rr_interval * 0.6) # 0.6x = slightly faster than real-time

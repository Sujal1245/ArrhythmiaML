...
X_feat = data['X_feat']    # HRV features  → Random Forest
X_seq  = data['X_seq']     # RR sequences  → 1D-CNN
y      = data['y']

print(f"Dataset loaded : {len(y)} windows")
print(f"Normal (0)     : {np.sum(y == 0)}")
print(f"AFib   (1)     : {np.sum(y == 1)}\n")

# ── Train / val / test split ──────────────────────────────────────────────
X_f_tr, X_f_te, X_s_tr, X_s_te, y_tr, y_te = train_test_split(
    X_feat, X_seq, y, test_size=0.2, random_state=42, stratify=y
)
X_f_tr, X_f_val, X_s_tr, X_s_val, y_tr, y_val = train_test_split(
    X_f_tr, X_s_tr, y_tr, test_size=0.1, random_state=42, stratify=y_tr
)

print(f"Train : {len(y_tr)} | Val : {len(y_val)} | Test : {len(y_te)}\n")

# ─────────────────────────────────────────────────────────────────────────
# MODEL 1 — Random Forest on HRV features
# ─────────────────────────────────────────────────────────────────────────
print("=" * 50)
...
joblib.dump(rf,     'afib_rf_model.pkl')
joblib.dump(scaler, 'afib_scaler.pkl')
print("Saved → afib_rf_model.pkl")
print("Saved → afib_scaler.pkl\n")

# ── Feature importance plot ───────────────────────────────────────────────
FEATURE_NAMES = [
    'Mean RR', 'SDNN', 'RMSSD', 'CV', 'pNN50',
    'Max RR', 'Min RR', 'Range RR',
    'Shannon Entropy', 'IEF', 'SD1', 'SD2', 'Window Length'
]
...
# ─────────────────────────────────────────────────────────────────────────
# MODEL 2 — 1D-CNN on raw RR sequences
# ─────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 50)
print("Training 1D-CNN...")
print("=" * 50)

# Normalise RR sequences using training stats only
rr_mean   = X_s_tr.mean()
rr_std    = X_s_tr.std()
...
np.save('afib_rr_stats.npy', np.array([rr_mean, rr_std]))
...
plt.tight_layout()
plt.savefig('afib_cnn_training.png', dpi=150)
plt.show()

cnn.save('afib_cnn_model.keras')
print("\nSaved → afib_cnn_model.keras")
print("Saved → best_afib_cnn.keras")
print("\nTraining complete!")

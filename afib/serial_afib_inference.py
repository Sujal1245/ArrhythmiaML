...
BAUD       = 115200
WINDOW     = 30          # beats per classification window
MIN_RR     = 300         # ms — filters out noise (>200 bpm)
MAX_RR     = 2000        # ms — filters out missed beats (<30 bpm)
...
ax_rf.set_xlim(0, 1)
ax_rf.set_title('Random Forest confidence',
                color='#cccccc', fontsize=9, pad=4)
...
ax_cnn.set_xlim(0, 1)
ax_cnn.set_title('1D-CNN confidence',
                 color='#cccccc', fontsize=9, pad=4)
...
try:
    while True:
        # ── Read one RR interval from serial ─────────────────────────
        try:
            line = ser.readline().decode('utf-8').strip()
        except UnicodeDecodeError:
            continue

        if line == '' or line == 'READY' or line == 'ERROR: MAX30100 not found.':
            continue
        ...
        # Update rolling RR plot on every beat
        ...
        # Need full window before classifying
        ...
        scat_rf  = ax_hist.scatter(
            [h[0] for h in history_rf],
            [h[1] for h in history_rf],
            c=[h[2] for h in history_rf], s=35, zorder=3)
        scat_cnn = ax_hist.scatter(
            [h[0] for h in history_cnn],
            [h[1] for h in history_cnn],
            c=[h[2] for h in history_cnn],
            s=35, zorder=3, marker='^')

        # ── Update counts ─────────────────────────────────────────────
        rf_counts[rf_pred]   += 1
        cnn_counts[cnn_pred] += 1
        for bar, val in zip(rf_cbars,  rf_counts):
            bar.set_height(val)
        for bar, val in zip(cnn_cbars, cnn_counts):
            bar.set_height(val)
        ...
        pred_label.set_bbox(dict(
            boxstyle='round,pad=0.3',
            facecolor=COLORS[cnn_pred], alpha=0.85))
